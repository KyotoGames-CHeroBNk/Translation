...
init python:
    ...
    class PoemWord:
        def __init__(self, word, sPoint, iPoint, ePoint, uPoint, aPoint, nPoint, yPoint, kPoint, rPoint, glitch=False):
            self.word = word
            self.sPoint = sPoint
            self.nPoint = nPoint
            self.yPoint = yPoint
...
label poem(transition=True):
    ...
    if persistent.playthrough == 0 and chapter == 0:
        call screen dialog("Пришло время написать стихотворение!\n\nВыберите слова, которые, по вашему мнению, понравятся вашему любимому члену\nклуба.С тем, кому ваше стихотворение понравится больше всего,\nможет случиться что-то хорошее!", ok_action=Return())
    python:
        ...
        while True:
            ...
            kPointTotal += t.kPoint
            rPointTotal += t.rPoint
            aPointTotal += t.aPoint
            ePointTotal += t.ePoint
            uPointTotal += t.uPoint
        ...
        if persistent.playthrough == 0:
            ...
            unsorted_pointlist = {"sayori": sPointTotal, "natsuki": nPointTotal, "yuri": yPointTotal, "kiba": kPointTotal, "int": iPointTotal, "und": uPointTotal, "exp": ePointTotal, "ath": aPointTotal, "rikka": rPointTotal,}
        ...
        if kPointTotal < POEM_DISLIKE_THRESHOLD: k_poemappeal[chapter] = -1
        elif kPointTotal > POEM_LIKE_THRESHOLD: k_poemappeal[chapter] = 1
        if rPointTotal < POEM_DISLIKE_THRESHOLD: r_poemappeal[chapter] = -1
        elif rPointTotal > POEM_LIKE_THRESHOLD: r_poemappeal[chapter] = 1
        ...
        if uPointTotal < POEM_DISLIKE_THRESHOLD: u_poemappeal[chapter] = -1
        elif uPointTotal > POEM_LIKE_THRESHOLD: u_poemappeal[chapter] = 1
        if ePointTotal < POEM_DISLIKE_THRESHOLD: e_poemappeal[chapter] = -1
        elif ePointTotal > POEM_LIKE_THRESHOLD: e_poemappeal[chapter] = 1
